"""Unit tests for priveil.advisor.model.build_advisor_model.
...
def test_anthropic_string_returned_as_is() -> None:
    settings = _settings(advisor_model="anthropic:claude-sonnet-4-6")
    result = build_advisor_model(settings)
    assert result == "anthropic:claude-sonnet-4-6"


def test_openai_string_returned_as_is() -> None:
    settings = _settings(advisor_model="openai:gpt-4o")
    result = build_advisor_model(settings)
    assert result == "openai:gpt-4o"


def test_bedrock_string_returned_as_is() -> None:
    settings = _settings(advisor_model="bedrock:anthropic.claude-sonnet-4-5-20250929-v1:0")
    result = build_advisor_model(settings)
...
def test_databricks_returns_openai_chat_model() -> None:
    ...
    settings = _settings(
        advisor_model="databricks-meta-llama-3-3-70b-instruct",
        advisor_base_url="https://adb-123.azuredatabricks.net/serving-endpoints",
        advisor_api_key="dapi-abc123",
    )
    result = build_advisor_model(settings)
...
def test_custom_endpoint_returns_openai_chat_model() -> None:
    ...
    settings = _settings(
        advisor_model="my-deployment",
        advisor_base_url="https://custom.endpoint/v1",
...
def test_no_model_raises_value_error() -> None:
    settings = _settings()  # advisor_model defaults to None
    with pytest.raises(ValueError, match="PRIVEIL_ADVISOR_MODEL must be set"):
        build_advisor_model(settings)


def test_base_url_without_api_key_uses_local_default() -> None:
    settings = _settings(
        advisor_model="some-deployment",
        advisor_base_url="https://adb-123.azuredatabricks.net/serving-endpoints",
    )
    result = build_advisor_model(settings)
    from pydantic_ai.models.openai import OpenAIChatModel
...
def test_api_key_without_base_url_uses_builtin_path() -> None:
    """advisor_api_key alone (no base_url) is harmless — treated as built-in provider."""
    settings = _settings(
        advisor_model="anthropic:claude-sonnet-4-6",
        advisor_api_key="ignored-without-base-url",
    )
    result = build_advisor_model(settings)
