...
unchanged — but the mode='advisor' refiner path runs end-to-end without error.
"""
...
async def test_detect_refine_true_runs_without_error(refined_client: AsyncClient) -> None:
    resp = await refined_client.post("/detect", json={"text": "Jane Smith jane@example.com", "mode": "advisor"})
...
async def test_detect_no_refiner_refine_true_silently_skips(detect_client: AsyncClient) -> None:
    """When no refiner is configured, mode="advisor" is silently ignored."""
...
async def test_detect_mode_defaults_to_judge(detect_client: AsyncClient) -> None:
    """Omitting mode should default to 'advisor' (silently skipped without refiner)."""
...
async def test_pseudonymise_refine_true_runs_without_error(refined_client: AsyncClient) -> None:
    resp = await refined_client.post("/pseudonymise", json={"text": "Jane Smith TFN 123 456 782", "mode": "advisor"})
    assert resp.status_code == 200
