...
async def test_pseudonymise_auto_detects_email(pseudonymise_client: AsyncClient) -> None:
    ...
    assert "billing@acme.com" not in resp.json()["data"]["anonymised_text"]


async def test_pseudonymise_with_prior_detections(pseudonymise_client: AsyncClient) -> None:
    """Passing pre-computed detections (body['data']) skips re-detection."""
    ...
    anon_resp = await pseudonymise_client.post(
        "/pseudonymise", json={"text": text, "detections": detect_resp.json()["data"]}
    )
    assert anon_resp.status_code == 200
    body = anon_resp.json()
    assert "123 456 782" not in body["data"]["anonymised_text"]
    assert "***-***-***" in body["data"]["anonymised_text"]
...
async def test_pseudonymise_response_shape(pseudonymise_client: AsyncClient) -> None:
    ...
    assert "anonymised_text" in body["data"]
    assert "entity_map" in body["data"]
    assert isinstance(body["data"]["entity_map"], dict)
...
async def test_pseudonymise_clean_text_unchanged(pseudonymise_client: AsyncClient) -> None:
    ...
    assert resp.status_code == 200
    assert "***-***-***" not in resp.json()["data"]["anonymised_text"]
    assert "XXX-XXX" not in resp.json()["data"]["anonymised_text"]
...
async def test_pseudonymise_au_bsb_replaced(pseudonymise_client: AsyncClient) -> None:
    ...
    assert anon_resp.status_code == 200
    assert "062-000" not in anon_resp.json()["data"]["anonymised_text"]
    assert "XXX-XXX" in anon_resp.json()["data"]["anonymised_text"]
